...
export default function MarketResearchPage() {
  ...
  // Market Opportunities (scored)
  const [opportunities, setOpportunities] = useState<Opportunity[]>([])
  const [newOpportunity, setNewOpportunity] = useState({ name: '', marketSize: '', growthRate: '', competition: '', feasibility: '' })

  // Competitive Positioning (price, quality, innovation per competitor)
  const [positioning, setPositioning] = useState<Record<string, { price: number; quality: number; innovation: number }>>({})

  // Benchmarks
  const [benchmarks, setBenchmarks] = useState<Benchmark[]>([])
  const [newBenchmark, setNewBenchmark] = useState({ metric: '', yourValue: '', industryAverage: '', topPerformer: '', unit: '' })
  ...
  const addOpportunity = () => {
    if (!newOpportunity.name.trim()) {
      showToast('Please enter opportunity name', 'error')
      return
    }
    const marketSize = parseFloat(newOpportunity.marketSize) || 0
    const growthRate = parseFloat(newOpportunity.growthRate) || 0
    const competition = Math.min(10, Math.max(1, parseFloat(newOpportunity.competition) || 5))
    const feasibility = Math.min(10, Math.max(1, parseFloat(newOpportunity.feasibility) || 5))
    const score = Math.round((growthRate / 10 + (10 - competition) / 2 + feasibility / 2) * 10)
    ...
    setNewOpportunity({ name: '', marketSize: '', growthRate: '', competition: '', feasibility: '' })
    showToast('Opportunity added!', 'success')
  }

  const updatePositioning = (competitorId: string, key: 'price' | 'quality' | 'innovation', value: number) => {
    setPositioning(prev => ({
      ...prev,
      [competitorId]: {
        price: prev[competitorId]?.price ?? 5,
        quality: prev[competitorId]?.quality ?? 5,
        innovation: prev[competitorId]?.innovation ?? 5,
        [key]: value,
      },
    }))
  }
  ...
  const addBenchmark = () => {
    ...
    const benchmark: Benchmark = {
      id: Date.now().toString(),
      metric: newBenchmark.metric.trim(),
      yourValue: parseFloat(newBenchmark.yourValue) || 0,
      industryAverage: parseFloat(newBenchmark.industryAverage) || 0,
      topPerformer: parseFloat(newBenchmark.topPerformer) || 0,
      unit: newBenchmark.unit || '',
    }
    setBenchmarks([...benchmarks, benchmark])
    setNewBenchmark({ metric: '', yourValue: '', industryAverage: '', topPerformer: '', unit: '' })
  ...
  const addJourneyStage = () => {
    ...
    setJourneyStages([...journeyStages, stage])
    setNewStage({ name: '' })
    showToast('Stage added!', 'success')
  }

  const loadTemplate = (template: 'saas' | 'ecommerce' | 'fintech') => {
    const names = { saas: 'SaaS', ecommerce: 'E-commerce', fintech: 'FinTech' }
    setResearchQuery(`${names[template]} Market Research`)
    showToast(`${names[template]} template loaded. Add your data to get started.`, 'success')
  }
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <label className="cursor-pointer inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg border-2 border-primary-500/30 text-primary-600 hover:bg-primary-50 transition-all duration-200">
                <input
                  type="file"
                  accept=".json"
                  onChange={importData}
            ...
            <Card>
              ...
              <div className="space-y-4 mb-4">
                <Select
                  ...
                  onChange={(e) => setNewQuestion({ ...newQuestion, type: e.target.value as any, options: e.target.value === 'multiple-choice' ? [''] : [] })}
                  options={[
